...
class Model(object):
    ...
    def make_model(self, s, trf, trf_cp, lb_now, lb_max, ln_now, ln_max, lnn_now):
        """
        ...
        v_y  = {(d,i,j): self.model.addVar(vtype='I', name='y.{}.{}.{}'.format(d,i,j)) for d in self.graph.site_list for i in N for j in N}
        ...
        for d,i in di_set:
            if i == s:
                self.model.addConstr(grb.quicksum(v_y[d,i,j] for j in N) - grb.quicksum(v_y[d,j,i] for j in N) ==  1)
            if i == d:
                self.model.addConstr(grb.quicksum(v_y[d,i,j] for j in N) - grb.quicksum(v_y[d,j,i] for j in N) == -1)
            if i != s and i != d:
                self.model.addConstr(grb.quicksum(v_y[d,i,j] for j in N) - grb.quicksum(v_y[d,j,i] for j in N) ==  0)
        ...
        # 制約: リンク数制限
        for i,j in ij_set:
            self.model.addConstr(grb.quicksum(v_x[d,i,j] + v_y[d,i,j] for d in D) + ln_now[i,j] >= 0)
            self.model.addConstr(grb.quicksum(v_x[d,i,j] + v_y[d,i,j] for d in D) + ln_now[i,j] <= ln_max)

        # 制約: リンク帯域制限
        for i,j in ij_set:
            self.model.addConstr(grb.quicksum(v_x[d,i,j] * trf + v_y[d,i,j] * trf_cp[d] for d in D) + lb_now[i,j] >= 0)
            self.model.addConstr(grb.quicksum(v_x[d,i,j] * trf + v_y[d,i,j] * trf_cp[d] for d in D) + lb_now[i,j] <= lb_max)
        
        # 制約: ノード処理量制限
        for j in N:
            self.model.addConstr(grb.quicksum(v_x[d,i,j] + v_y[d,i,j] for d in D for i in N) + lnn_now[j] >= 0)
            self.model.addConstr(grb.quicksum(v_x[d,i,j] + v_y[d,i,j] for d in D for i in N) + lnn_now[j] <= lb_max)

        # 制約: 各リンクの未使用本数の最小
        for i,j in ij_set:
            if ln_now[i,j] >= ln_max:
                continue
            self.model.addConstr(ln_max - (grb.quicksum(v_x[d,i,j] + v_y[d,i,j] for d in D) + ln_now[i,j]) >= v_mn)
        
        # 制約: 各リンクの未使用帯域の使用帯域の最小
        for i,j in ij_set:
            if lb_now[i,j] >= lb_max:
                continue
            self.model.addConstr(lb_max - (grb.quicksum(v_x[d,i,j] * trf + v_y[d,i,j] * trf_cp[d] for d in D) + lb_now[i,j]) >= v_mb)
    ...
    def optimize(self, s, trf, trf_cp, lb_now, lb_max, ln_now, ln_max, nn_now, info=False):
        """
        モデルを作成し，最適化を実行する．
        もし，infoがTrueなら最適化した経路を表示する．
        """

        self.make_model(s, trf, trf_cp, lb_now, lb_max, ln_now, ln_max, nn_now)

        self.model.setParam('OutputFlag', False)

        self.model.optimize()
        self.make_route(s)
        self.make_route_cp(s)
    ...
    def make_route_cp(self, s):
        ...
        L = self.graph.both_link_list
        N = self.graph.node_num
        allocate = {d: [(i, j) for i, j in L if self.Y[d,i,j].x > 0] for d in D}
        route    = {d: list() for d in self.graph.site_list}
                ...
                if i in passed:
                    ...
                    while True:
                        ...
                        if poped[0] in passed:
                            ...
                            break
            route[d] = copy.deepcopy(tmp)
        self.route_cp[s] = copy.deepcopy(route)
