...
class Model(object):
    ...
    def make_model(self, s, l_use):
        ...
        Tsig = self.DATA.signal_lb / self.DATA.graph.site_num
    ...
    def show_route(self, s):
        ...
        構築した経路を表示する．
        """
        idx = [(i,j) for i in range(self.DATA.graph.site_list) for j in range(self.DATA.graph.node_num)]
        for d in self.DATA.graph.site_list:
            ...
            p = s
            while(True):
                for i in range(self.DATA.graph.site_list):
                    if (p,i) in is_link:
                        print "({}, {})".format(s,i),
                        p = i
                        break
...
if __name__ == '__main__':
    ...
    data  = Data.Data(graph, 10000, 1000,10,200)
    model = Model(data)
