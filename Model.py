...
class Model(object):
    ...
    def make_model(self, s, trf, lb_now, lb_max, ln_now, ln_max):
        """
        ...
        # 目的関数
        self.model.setObjective(v_mn + v_mb * 200 / lb_max, grb.GRB.MAXIMIZE)
        ...
        for i,j in ij_set:
            if ln_now[i,j] >= ln_max:
                continue
            self.model.addConstr(ln_max - (grb.quicksum(v_x[d,i,j] for d in D) + ln_now[i,j]) >= v_mn, 'const:M_num')
        
        # 制約: 各リンクの未使用帯域の使用帯域の最小
        for i,j in ij_set:
            if lb_now[i,j] >= lb_max:
                continue
            self.model.addConstr(lb_max - (grb.quicksum(v_x[d,i,j] * trf for d in D) + lb_now[i,j]) >= v_mb, 'const:M_band')
    ...
    def optimize(self, s, trf, lb_now, lb_max, ln_now, ln_max, info=False):
        """
        モデルを作成し，最適化を実行する．
        もし，infoがTrueなら最適化した経路を表示する．
        """
        self.make_model(s, trf, lb_now, lb_max, ln_now, ln_max)
        self.model.optimize()
        self.make_route(s)
