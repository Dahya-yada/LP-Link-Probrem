...
class Model(object):
    ...
    def make_model(self, s, trf_sig, trf_cp, lb_now, lb_max, ln_now, ln_max, nw_node_now):
        ...
        # 制約: リンク帯域制限
        for i,j in ij_set:
            self.model.addConstr(grb.quicksum(v_x[d,i,j] * trf_sig + v_y[d,i,j] * trf_cp[d] for d in D) + lb_now[i,j] >= 0)
            self.model.addConstr(grb.quicksum(v_x[d,i,j] * trf_sig + v_y[d,i,j] * trf_cp[d] for d in D) + lb_now[i,j] <= lb_max)

        # 制約: ノード処理量制限
        for j in N:
            self.model.addConstr(grb.quicksum(v_x[d,i,j] + v_y[d,i,j] for d in D for i in N) + nw_node_now[j] >= 0)
            self.model.addConstr(grb.quicksum(v_x[d,i,j] + v_y[d,i,j] for d in D for i in N) + nw_node_now[j] <= lb_max)

        # 制約: 各リンクの未使用本数の最小
        for i,j in ij_set:
            if ln_now[i,j] >= ln_max:
                continue
            self.model.addConstr(ln_max - (grb.quicksum(v_x[d,i,j] + v_y[d,i,j] for d in D) + ln_now[i,j]) >= v_n)

        # 制約: 各リンクの未使用帯域の使用帯域の最小
        for i,j in ij_set:
            if lb_now[i,j] >= lb_max:
                continue
            self.model.addConstr(lb_max - (grb.quicksum(v_x[d,i,j] * trf_sig + v_y[d,i,j] * trf_cp[d] for d in D) + lb_now[i,j]) >= v_l)
    ...
    def optimize(self, s, trf_sig, trf_cp, lb_now, lb_max, ln_now, ln_max, nw_node_now, info=False):
        ...
        if info:
            ...
            print('<Route SIG>')
            self.show_route(s, self.route_sig)
            print('<Route CP>')
            self.show_route(s, self.route_cp)
